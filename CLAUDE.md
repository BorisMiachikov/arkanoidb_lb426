...
- **NameInput** — буфер ввода имени (макс. 10 ASCII-символов). Ввод через `EventReader<KeyboardInput>`: `Key::Character(SmolStr)` для букв, `Key::Backspace`, `Key::Enter`, `Key::Escape`. Импорт: `use bevy::input::keyboard::{Key, KeyboardInput}; use bevy::input::ButtonState;`
